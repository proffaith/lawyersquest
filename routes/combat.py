...
def ajax_handle_combat():
    ...
    def sp(path):  # build static URL
        return url_for('static', filename=path)

    # enemy values pulled from session with safe defaults
    def enemy_state(key, default_path, default_frames=1):
        return {
            "sheet": sp(enemy.get(key, default_path)),
            "frames": int(enemy.get(f"frames_{key.split('_')[-1]}", default_frames)),
            "fps":    int(enemy.get("fps", 12)),
            "w":      int(enemy.get("frame_w", 128)),
            "h":      int(enemy.get("frame_h", 128)),
        }

    SQUIRE_IDLE   = {"sheet": sp('sprites/squire_idle.jpeg'),  "frames": 1, "fps": 12, "w": 256, "h": 256}
    SQUIRE_ATTACK = {"sheet": sp('sprites/squire_attack.jpeg'),"frames": 1, "fps": 12, "w": 256, "h": 256}
    SQUIRE_HURT   = {"sheet": sp('sprites/squire_hurt.jpeg'),  "frames": 1, "fps": 12, "w": 256, "h": 256}
    ...
    if action == "flee":
        ...
        battle_log = []
        # inside ajax_handle_combat(), after you compute hit/miss
        anim = {
            "squire": {"state": "idle", **SQUIRE_IDLE},
            "enemy":  {"state": "idle", **enemy_state("sprite_idle", "sprites/tax_auditor/idle.png")}
        }
        ...
        if random.randint(1, 100) <= hit_chance:
            ...
            battle_log.append(
                f"💥 You hit the {enemy['name']} with your {enemy['weakness']}! They are getting hungrier."
            )
            # squire attacks; enemy staggers
            anim["squire"].update({"state": "attack", **SQUIRE_ATTACK})
            anim["enemy"].update({"state": "stagger", **enemy_state("sprite_stagger", "sprites/tax_auditor/stagger.png", default_frames=1)})
        else:
            player_current_hunger += 1
            battle_log.append(f"❌ Ouch! Bad Touch, {enemy['name']}! You are getting hungrier.")
            # enemy attacks; squire is hurt
            anim["enemy"].update({"state": "attack", **enemy_state("sprite_attack", "sprites/tax_auditor/attack.png", default_frames=1)})
            anim["squire"].update({"state": "hurt", **SQUIRE_HURT})
...
def encounter_enemy():
    ...
    squire_id = flask_session.get("squire_id")
    def sprite(relpath): return url_for('static', filename=relpath)
    if not squire_id:
    ...
    try:
        ...
        if enemy:
            ...
            slug = (enemy.enemy_name or "enemy").lower().replace(" ", "_")
            ...
            flask_session['enemy'] = {
                ...
                # ✅ store RELATIVE sprite paths (under /static)
                "sprite_idle":    enemy.sprite_idle    or f"sprites/{slug}/idle.png",
                "sprite_attack":  enemy.sprite_attack  or f"sprites/{slug}/attack.png",
                "sprite_stagger": enemy.sprite_stagger or f"sprites/{slug}/stagger.png",
                "sprite_hurt":    enemy.sprite_hurt    or f"sprites/{slug}/hurt.png",

                # per-frame metadata (per-frame sizes, not sheet size)
                "frame_w":        enemy.frame_w or 128,
                "frame_h":        enemy.frame_h or 128,
                "frames_idle":    enemy.frames_idle or 1,
                "frames_attack":  enemy.frames_attack or 1,
                "frames_stagger": enemy.frames_stagger or 1,
                "frames_hurt":    enemy.frames_hurt or 1,
                "fps":            enemy.fps or 12,
            }
...
def encounter_boss():
    ...
    try:
        ...
        if quest_id == 14:
            ...
            boss_q = (
                db.query(Enemy)
                  .filter(Enemy.is_boss.is_(True))
            )

            boss = boss_q.order_by(func.rand()).first()
...
@combat_bp.route('/tournament', methods=['GET', 'POST'])
def tournament():
    return redirect(url_for('questions.answer_MC_question'))
