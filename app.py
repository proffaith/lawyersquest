...
from sqlalchemy import create_engine, func, and_, exists
...
MAIL_SERVER = "smtp.us-west-1.mailertogo.net"
MAIL_PORT = 587
MAIL_USE_TLS = True
MAIL_USERNAME = os.environ["MTG_SMTP_USER"]
MAIL_PASSWORD = os.environ["MTG_SMTP_PASS"]
MAIL_DEFAULT_SENDER = ("Lawyer's Quest", "noreply@proffaith.com")
...
from utils.shared import generate_terrain_features_with_roads
from utils.shared import calculate_feature_counts_advanced
from utils.shared import get_existing_features_from_db
from utils.shared import enhanced_quest_start_with_stability_check
...
from routes.skills import get_skill_state
...
# 1) Put values in app.config (not app.MAIL_*)
app.config.update(
    MAIL_SERVER=os.environ.get("MAILERTOGO_SMTP_HOST", "smtp.us-west-1.mailertogo.net"),
    MAIL_PORT=int(os.environ.get("MAILERTOGO_SMTP_PORT", 587)),
    MAIL_USE_TLS=True,
    MAIL_USERNAME=os.environ["MTG_SMTP_USER"],
    MAIL_PASSWORD=os.environ["MTG_SMTP_PASS"],
    MAIL_DEFAULT_SENDER=("Lawyer's Quest", "noreply@proffaith.com"),
...
def send_verification_email(squire_email: str, squire_name: str, token: str) -> bool:
    confirm_url = f"https://lawyersquest.proffaith.com/verify?token={token}"
    try:
        msg = Message(
            subject="Confirm your registration for Lawyer's Quest",
            recipients=[squire_email],
            body=f"Hail, noble {squire_name}! \n\n(Flask-Mail via MTG SMTP)",
            html=f"""
        ...
        logging.exception("SMTPException occurred")  # logs stacktrace + server codes
    except Exception as e:
        logging.exception("Unexpected error in send_verification_email")
    return False
...
def inject_version():
    return {"app_version": flask_session.get("ver", "0.3.10")}
...
def home():
    ...
    try:
        ...
        # Optional: ensure we always show something
        if not rows:
            logging.info("No enemies found; falling back to first two by level.")
            items = db.query(Enemy).order_by(Enemy.min_level.asc()).limit(2).all()
            rows = [{
                "id": e.id, "name": e.enemy_name, "level": e.min_level,
                "boss": e.is_boss, "weakness": e.weakness,
                "gold": e.gold_reward, "xp": e.xp_reward,
                "img": e.static_image, "desc": (e.description or "").strip()
            } for e in items]

    except Exception as e:
        logging.warning(f"Error getting featured enemies {e}")

    finally:
        db.close()

    return render_template("index.html", featured_bestiary=rows)
...
def start_quest():
    ...
    try:
        ...
        existing_features = get_existing_features_from_db(db, squire_id)
        treesize, mountainsize, riversize, zone_info = calculate_feature_counts_advanced(
            level=level,
            quest_id=quest_id,
            existing_features=existing_features
            )

        stability_check = enhanced_quest_start_with_stability_check(db, squire_id, squire_quest_id)
        if stability_check['recommendation'] == 'add_terrain':

            generate_terrain_features_with_roads(
            ...
            completed_qids = (
                db.query(SquireQuestStatus.quest_id)
                  .filter(
                    ...
                    SquireQuestStatus.status.in_(("completed","locked"))
                    )
                  .distinct()
                  .subquery()
            )
