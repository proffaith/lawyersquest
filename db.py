...
from sqlalchemy import  (
    create_engine, Column, Integer, String, DateTime, Text, ForeignKey, func, Enum, Boolean, UniqueConstraint,
    CheckConstraint
    )
...
class Squire(Base):
    ...
    skill_points = relationship(
        "SquireSkillPoint",
        back_populates="squire",
        cascade="all, delete-orphan",
        foreign_keys="SquireSkillPoint.squire_id",
    )
...
class Quest(Base):
    ...
    curriculum_key     = Column(String(64), nullable=True)
...
class Enemy(Base):
    ...
    static_image = Column(String(255), nullable=True)
    sprite_idle = Column(String(255), nullable=True)
    sprite_attack = Column(String(255), nullable=True)
    sprite_stagger = Column(String(255), nullable=True)
    sprite_hurt = Column(String(255), nullable=True)
    frame_w = Column(Integer, default=128, nullable=True)
    frame_h = Column(Integer, default=128, nullable=True)
    frames_idle = Column(Integer, default=1, nullable=True)
    frames_attack = Column(Integer, default=1, nullable=True)
    frames_stagger = Column(Integer, default=1, nullable=True)
    frames_hurt = Column(Integer, default=1, nullable=True)
    fps = Column(Integer, default=12, nullable=True)

    true_false_questions = relationship('TrueFalseQuestion',back_populates='enemy', cascade='all, delete-orphan')

    skill_susceptibilities = relationship(
        "EnemySkillSusceptibility",
        back_populates="enemy",
        cascade="all, delete-orphan",
        foreign_keys="EnemySkillSusceptibility.enemy_id",
    )
...
class MapNode(Base):
    ...
    node_type = Column(Enum('waypoint','quest','dungeon','boss_arena','tournament'), nullable=False)
    quest_id = Column(Integer)
...
class Encounter(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    slug = Column(String(64))
    encounter_type = Column(Enum('boss','tournament','dungeon'), nullable=False)
    title = Column(String(128))
    description_md = Column(Text)
    difficulty = Column(Enum('easy', 'normal', 'hard', 'legendary'),default = 'normal')
    rules_json = Column(Text)
    reward_json = Column(Text)

class EncounterStage(Base):
    __tablename__ = "EncounterStage"

    id = Column(Integer, primary_key=True, autoincrement=True)
    encounter_id = Column(Integer)
    stage_index = Column(Integer)
    stage_rules = Column(Text)
...
class Skill(Base):
    """
    Master list of investable skills (e.g., 'oral_argument', 'paper_shredder').
    """
    __tablename__ = "Skill"
    __table_args__ = (
        UniqueConstraint("code", name="uq_skill_code"),
        CheckConstraint("max_points >= 0", name="ck_skill_max_points_nonneg"),
    )
    ...
    # Relationships
    enemy_links: Mapped[List["EnemySkillSusceptibility"]] = relationship(
        back_populates="skill",
        cascade="all, delete-orphan",
    )
    player_points: Mapped[List["SquireSkillPoint"]] = relationship(
        back_populates="skill",
        cascade="all, delete-orphan",
    )

    def __repr__(self) -> str:
        return f"<Skill {self.code} (max={self.max_points})>"


# --- EnemySkillSusceptibility -------------------------------------------------

class EnemySkillSusceptibility(Base):
    """
    Join table: which enemies are affected by which skills, with a weight multiplier.
    Composite PK keeps it compact and enforces uniqueness.
    """
    __tablename__ = "EnemySkillSusceptibility"
    __table_args__ = (
        CheckConstraint("weight BETWEEN 1 AND 200", name="ck_ess_weight_range"),
    )

    enemy_id: Mapped[int] = mapped_column(
        ForeignKey("enemies.id", ondelete="CASCADE"), primary_key=True
    )
    skill_id: Mapped[int] = mapped_column(
        ForeignKey("Skill.id", ondelete="CASCADE"), primary_key=True
    )

    # 50–150 typical; constraint allows 1–200 to give you future wiggle room
    weight   = mapped_column(MySQLTinyInt(unsigned=True), nullable=False, default=100)

    # Relationships
    enemy: Mapped["Enemy"] = relationship(back_populates="skill_susceptibilities")
    skill: Mapped["Skill"] = relationship(back_populates="enemy_links")

    def __repr__(self) -> str:
        return f"<ESS enemy={self.enemy_id} skill={self.skill_id} weight={self.weight}>"


# --- SquireSkillPoint ---------------------------------------------------------

class SquireSkillPoint(Base):
    __tablename__ = "SquireSkillPoint"  # you can keep this; table names don't have to match class names

    __table_args__ = (
        # This UniqueConstraint is redundant because the same columns are PKs,
        # but keeping it won't hurt. You can remove it if you like.
        UniqueConstraint("squire_id", "skill_id", name="uq_squire_skill"),
        CheckConstraint("points >= 0", name="ck_psp_points_nonneg"),
    )

    # 🔧 Fix the FK target to EXACT table/column names
    squire_id: Mapped[int] = mapped_column(
        ForeignKey("squires.id", ondelete="CASCADE"),  # <-- was "Squires.id"
        primary_key=True,
    )
    # Do the same check for Skill: is your table actually "Skill" or "skills"?
    skill_id: Mapped[int] = mapped_column(
        ForeignKey("Skill.id", ondelete="CASCADE"),    # <-- change to "skills.id" if that's your real table
        primary_key=True,
    )
    ...
    # Be explicit about targets + fkeys; avoids any inference weirdness
    squire: Mapped["Squire"] = relationship(
        "Squire",
        back_populates="skill_points",
        foreign_keys=[squire_id],
    )
    skill: Mapped["Skill"] = relationship(
        "Skill",
        back_populates="player_points",
        foreign_keys=[skill_id],
    )

    def __repr__(self) -> str:
        return f"<SquireSkillPoint squire={self.squire_id} skill={self.skill_id} points={self.points}>"


# --- LevelSkillGrant ----------------------------------------------------------

class LevelSkillGrant(Base):
    """
    How many skill points are granted when a player reaches a given level.
    (e.g., level 1..99 -> 2 points each by default)
    """
    __tablename__ = "LevelSkillGrant"
    __table_args__ = (
        CheckConstraint("level >= 1", name="ck_lsg_level_min"),
        CheckConstraint("points_granted >= 0", name="ck_lsg_points_nonneg"),
    )

    level: Mapped[int] = mapped_column(Integer, primary_key=True)
    points_granted: Mapped[int] = mapped_column(Integer, nullable=False, default=2)

    def __repr__(self) -> str:
        return f"<LevelSkillGrant lvl={self.level} points={self.points_granted}>"
